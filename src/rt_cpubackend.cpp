...
void recv_and_proc(uhd::usrp::multi_usrp::sptr usrp,
    ...
    float fcenter,
    float bw, 
    size_t    observatoryid,
    size_t    ibeam,
    size_t    nbeam,
    float    RAD,
    float    DecD,
    const std::string& time_domain_dump_name="",
    const std::string& freq_domain_dump_name="",
    const std::string& specfile_name="",
    ...
    )
{
    ...
    //printf("setting up the shm nch %ld batch %ld nbatch %ld\\n", nch, batch,
            //nbatch);
    ...
    std::function<void(const DataFrame& df)> handler_f;

    std::ofstream time_domain_dump_file;
    std::ofstream freq_domain_dump_file;

    if(time_domain_dump_name.empty()){
        handler_t=[](const DataFrame&){};
    }else{
        std::cerr<<"dump raw baseband to " << time_domain_dump_name <<std::endl;
    ...
    if(freq_domain_dump_name.empty() && specfile_name.empty()){

    handler_f=[&](const DataFrame& df){
        int cnt=df.count;
    ...
    else if(specfile_name.empty()){
        std::cerr<<"dump filterbank file to "<< freq_domain_dump_name <<std::endl;
        const int nbit = 32;
        double tsamp_in = nch/bw;
        double fch1 = (fcenter + bw/2.)*1.e-6;
        double foff = bw/nch*1.e-6;
        write_header(freq_domain_dump_name.c_str(), sourcename.c_str(), 8, observatoryid, nch, nbit, nbeam, ibeam, MJD - fmod(MJD,  1.), fmod(MJD , 1.),  tsamp_in, fch1, foff, 0., 0., RAD, DecD);

        //printf("we wrote the header");

        freq_domain_dump_file.open(freq_domain_dump_name.c_str(), std::ios::app);
        //freq_domain_dump_file.open(freq_domain_dump_name.c_str());
        //FILE *freq_domain_dump_file;
        //freq_domain_dump_file = fopen(freq_domain_dump_name.c_str(), "wb+");
        
//printf("we open the file again");
    ...
    handler_f=[&](const DataFrame& df){
        ...
        for(int i=0;i<df.payload.size();++i){
            ...
            //write(freq_domain_dump_file, (char*)base_ptr, df.payload.size()*sizeof(float));
            //write( fileno(freq_domain_dump_file), (char*)base_ptr, df.payload.size()*sizeof(float));
            
            freq_domain_dump_file.write((char*)base_ptr, df.payload.size()*sizeof(float));
            //freq_domain_dump_file.flush();
    };

    }
    else {
        std::cerr<<"dump spectrum to "<< specfile_name <<std::endl;
        specfile_file.open(specfile_name.c_str());
    ...
    handler_f=[&](const DataFrame& df){
        ...
        for(int i=0;i<df.payload.size();++i){
            base_ptr[i]=std::norm(df.payload[i]);
            int j = i % nch;
            specdata[j] += base_ptr[i];
        }
            specfile_file.write((char*)specdata, nch*sizeof(float));
    };
...
int UHD_SAFE_MAIN(int argc, char* argv[])
{
    ...
    size_t channel, nch, batch, observatoryid;
    size_t display_buf_nbatch, ibeam, nbeam;
    double rate, freq, gain, bw, setup_time, lo_offset;
    double RAD, DecD;
    std::string tdump, fftfile, specfile, sourcename;
    ...
    desc.add_options()
        ...
        ("nch", po::value<size_t>(&nch)->default_value(1024), "num of channels")
        ...
        ("filfile", po::value<std::string>(&fftfile)->default_value(""), "dump FFT to file")
        ("specfile", po::value<std::string>(&specfile)->default_value(""), "dump spectrum to file")
        ("sourcename", po::value<std::string>(&sourcename)->default_value("sourcename"), "name of the source")
        ("observatory_id", po::value<size_t>(&observatoryid)->default_value(21), "the observatory id")
        ("RAD", po::value<double>(&RAD)->default_value(0.0), "The RAD (deg) of the source")
        ("DecD", po::value<double>(&DecD)->default_value(0.0), "The DecD (deg) of the source")
        ("ibeam", po::value<size_t>(&ibeam)->default_value(1), "The beam number")
        ("nbeam", po::value<size_t>(&nbeam)->default_value(1), "The number of beams")
